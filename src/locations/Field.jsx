...
const joinFieldValuesToString = ({
  field = {},
  fieldIds = [],
  entryValues = [],
}) => {
  const result = fieldIds
    .map((fieldId) => entryValues[fieldId] || null)
    .filter((value) => value)
    .join(" ");
...
const Field = () => {
  const sdk = useSDK();
  const { isLoading, space, environment } = useCMA();
  const [entryValues, setEntryValues] = useState({});
  const locale = sdk.locales.default;
  const window = sdk.window;
  const fieldIds = useMemo(
    () =>
      sdk.parameters.instance.fieldIds
        .split(",")
        .map((fieldId) => fieldId.trim()),
    [sdk]
  );
  const fieldsMetadata = fieldIds
    .map((fieldId) => sdk.contentType.fields.find(({ id }) => id === fieldId))
    .filter((field) => field);
  ...
  const titleFieldCallback = useCallback(
    (values = [], fieldId = "") => {
      ...
      Promise.all(wrappedPromises)
        .then((titles) => {
          const newTitle = titles
            .filter((title) => title.status === "fulfilled" && title[locale])
    ...
    fields.forEach((field) => {
      ...
      if (
        ...
        field.onValueChanged(() => {
          const values =
            field.type === "Link" ? [field.getValue()] : field.getValue();
      ...
      field.onValueChanged(() => {
        if (field.type === "Date" && field.getValue()) {
          ...
          setEntryValues((prev) => ({
            ...prev,
            [field.id]: date.toLocaleDateString(locale, DATE_FORMAT),
          }));
          return;
        }
  ...
  const { joined, isSliced } = joinFieldValuesToString({
    field: sdk.field,
    fieldIds,
    entryValues,
  });
  ...
  if (
    joined !== sdk.field.getValue() &&
    fieldIds.length - notFound.length === Object.keys(entryValues).length
  ) {
  ...
  return (
    <Stack flexDirection="column" spacing="spacingS">
      ...
      {isSliced && (
        <Note variant="warning" style={{ width: "100%" }}>
          This field only allows max 256 characters. The generated title is
          longer so it was cut to fit 🔪
        </Note>
      )}
    </Stack>
  );
